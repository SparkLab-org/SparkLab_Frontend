...
export default function MenteeDetailView({ menteeId: menteeIdProp, showBackLink = true }: Props) {
  ...
  const menteeTodos = useMemo(() => {
    const hasAssignee = todos.some(
      (todo) => String(todo.assigneeId ?? '').length > 0 || String(todo.assigneeName ?? '').length > 0
    );
    const filtered = todos.filter((todo) => {
      if (!hasAssignee) return true;
      const byId = String(todo.assigneeId ?? '') === String(resolvedMenteeId);
      const byName = selectedMentee?.name ? String(todo.assigneeName ?? '') === selectedMentee.name : false;
      return byId || byName;
    });
    return filtered;
  ...
  }, [selectedMentee, resolvedMenteeId]);
